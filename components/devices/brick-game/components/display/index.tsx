...
export const BrickGameDisplay: FC<Props> = ({ matrix }) => {
  ...
  >
    ...
    {
      matrix.map((row, rowIndex) => row.map((cellValue, columnIndex) =>
        <g
          key={`${rowIndex}_${columnIndex}_${cellValue}`}
          transform={ `translate(${columnIndex * 10 + 8},${rowIndex * 10 + 6})` }
        >
          <DisplayCell isActive={cellValue} />
        </g>
      )).flat()
    }
    <defs>
      ...
      >
        ...
        <feMorphology
          in="SourceAlpha"
          radius={4}
          result="effect1_innerShadow_10_1281"
        />
        <feOffset dy={4} />
      ...
      >
        ...
        <feBlend
          in="SourceGraphic"
          in2="effect1_dropShadow_10_1281"
          result="shape"
        />
      </filter>
      <filter
